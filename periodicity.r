...
gtf_file = paste(static_dir, "gencode.v30.basic.annotation.gtf", sep="/")

bamfolder = paste(project_dir, "analysis/output/alignments_unsorted/periodicity", sep="/")
plot_dir = paste(project_dir, "analysis/output/figures/periodicity", sep="/")

heatmaps = paste(project_dir, "analysis/output/figures/periodicity/heatmap", sep="/")

p_sites_per_region = paste(project_dir, "analysis/output/figures/periodicity/p_sites_per_region", sep="/")
frames_path = paste(project_dir, "analysis/output/figures/periodicity/frames", sep="/")
metaprofile_path = paste(project_dir, "analysis/output/figures/periodicity/metaprofile", sep="/")
codon_usage_path = paste(project_dir, "analysis/output/figures/periodicity/codon_usage", sep="/")
...
for (filename in list.files(bamfolder, pattern=bam_pattern)) {
  print(filename)
...
for (filename in list.files(bamfolder, pattern="*.bam")) {
  samplename = file_path_sans_ext(filename)
  psite_region <- region_psite(reads_psite_list, annotation_dt, sample = samplename)
  ggsave(paste(p_sites_per_region, "/", samplename, ".jpeg", sep=""), plot=psite_region[["plot"]], width=10, height=7, units="in")
}
...
print("Metaprofile heatmap")
for (filename in list.files(bamfolder, pattern="*.bam")) {
  samplename = file_path_sans_ext(filename)
  output_file = paste(metaprofile_path, "/", "heatmap_", samplename, ".jpeg", sep="")
  
  comparison_dt <- list()
  subsample = paste(samplename, "28nt", sep="_")
  comparison_dt[[subsample]] <- reads_psite_list[[samplename]][length == 28]
  comparison_dt[["whole_sample"]] <- reads_psite_list[[samplename]]
  names_list <- list("Only_28" = c(subsample),
                     "All" = c("whole_sample"))
  metaheatmap <- metaheatmap_psite(comparison_dt, annotation_dt, sample = names_list,
                                   utr5l = 20, cdsl = 40, utr3l = 20, log = F)
  ggsave(output_file, device = NULL, plot=metaheatmap[["plot"]], width=10, height=7, units="in")
}
