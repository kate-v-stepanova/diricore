...
def main():
    ...
    ap.add_argument('-b', '--bamfile', help="bamfile path", required=True)
    ap.add_argument('-n', '--bamname', help="bamfile name", required=True)
    ...
    log.info("Processing BAM file '%s'" % args.bamname)
    counts = process_bam(args.bamfile, a, ts, tid2idx, tidlengths, args.min_aqual)
    write_counts(h5file, "/counts/%s" % args.bamname, counts)
    log.info("Processing BAM file '%s' [DONE]" % args.bamname)
