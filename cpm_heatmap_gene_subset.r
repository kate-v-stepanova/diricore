...
if (length(args) >= 3) {
  ...
  # infile_path = "/icgc/dkfzlsdf/analysis/OE0532/17246/analysis/input/metadata/cpm_heatmap_3.txt"
  infile_path = "/icgc/dkfzlsdf/analysis/OE0532/17246/analysis/input/ribo_waltz/8_10_samples.txt"
  selected_samples <- scan(infile_path, what=character())
  filename_preffix = "8_10"
...
if (length(args) >= 5) {
  infile_path = args[5]
  infile_path = "/icgc/dkfzlsdf/analysis/OE0532/17246/analysis/output/cpm_heatmap/2_4_6_selected_genes.tsv"
  selected_genes <- scan(infile_path, what=character())
...
if (length(selected_genes) != 0) {
  gene_names_dt <- gene_names_dt[gene_names_dt$gene_name %in% selected_genes,]
}
...
coding_dt <- as.data.frame(coding_dt)[,c(selected_samples, 'gene_name')]
noncoding_dt <- as.data.frame(noncoding_dt)[,c(selected_samples, 'gene_name')]

# rownames(coding_dt) <- coding_dt$gene_name
# coding_dt$gene_name <- NULL
# coding_dt <- as.data.frame(coding_dt)
# coding_dt[apply(coding_dt[,-1], 1, function(x) !all(x==0)),] # doesnt work bitch

# coding_dt <- coding_dt[apply(coding_dt[,-1], 1, function(x) !all(x==0)),] # remove rows with all zeros
# 
# rownames(noncoding_dt) <- noncoding_dt$gene_name
# noncoding_dt$gene_name <- NULL
# noncoding_dt <- as.data.frame(noncoding_dt)
# noncoding_dt <- apply(noncoding_dt[,-1], 1, function(x) !all(x==0)) # remove rows with all zeros
...
coding_cpm <- coding_cpm[as.logical(rowSums(coding_cpm >= 0)), ]
...
col_order <- c('gene_name', selected_samples)
coding_cpm <- coding_cpm[, col_order]
...
data_file = paste(data_dir, "/", filename_preffix, "cpm_top50_coding_genes.tsv", sep="")
write.table(top50_coding, file=data_file, sep="\t", quote = F, row.names = T)
...
outfile = paste(plot_dir, "/", filename_preffix, "heatmap_cpm_top50_coding.jpeg", sep="")
...
row_order = names(V[order(V, decreasing = F)])
...
col_order <- c('gene_name', selected_samples)
noncoding_cpm <- noncoding_cpm[, col_order]
...
print(paste("Saving file:", outfile))
outfile = paste(plot_dir, "/", filename_preffix, "heatmap_cpm_top50_noncoding.jpeg", sep="")
print(paste("Saving file:", outfile))
